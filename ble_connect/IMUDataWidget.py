...
class IMUDataWidget:
    def __init__(self, app, device, connect_button_callback, extra_id: str = ""):
        self.app = app
    ...
    def add_widget(self, container: str):
        ...
        with dpg.child_window(parent=container, tag=self.tag, auto_resize_y=True, auto_resize_x=True):
            with dpg.group(horizontal=True):
                ...
                dpg.add_button(tag=f"{self.tag}_button", label="Connect", callback=self.connect_button_callback, user_data=self.device, enabled=True, show=True)
